...
public class StockServiceImpl implements StockService {
	...
	public PaginatedResponse<PurchaseDto> getProductInboundDetail(int productId, int inboundPage, int size,
	...
	@Override
	@Transactional(readOnly = true)
	public PaginatedResponse<StockAdjustmentDto> getProductOutboundDetail(int productId, int outboundPage, int size,
			String startDate, String endDate) {
		if (productId <= 0) {
			throw new IllegalArgumentException("유효하지 않은 productId 입니다.");
		}
	...
	public void adjustProduct(int productId, StockAdjustRequestDto request) {
		...
		if ("ADD".equalsIgnoreCase(action)) { // 대소문자 무시하고 문자열 비교
			...
			stockParams.put("quantity", request.getQuantity());
			stockDao.updateProductStockAfterPurchase(stockParams);
			...
			stockParams.put("productId", productId);
			stockParams.put("quantity", request.getQuantity());
			stockDao.updateProductStockAfterAdjustment(stockParams);
